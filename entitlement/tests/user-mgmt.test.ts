...
import { tenants, companies, users, userCompanies, userPermissions, permissions } from '../src/db/schema'
...
test('DELETE /users/:id/companies/:companyId ถอน membership + สิทธิ์ scope company นั้น แต่สิทธิ์บริษัทอื่นอยู่ครบ', async () => {
  const { tenant, user } = await makeUserInTenant()
  const [coGone] = await db.insert(companies).values({ tenantId: tenant.id, name: 'Co D' }).returning()
  const [coStay] = await db.insert(companies).values({ tenantId: tenant.id, name: 'Co S' }).returning()
  const [perm] = await db.select().from(permissions).where(eq(permissions.key, 'esign.document.sign'))
  await db.insert(userCompanies).values([{ userId: user.id, companyId: coGone.id }, { userId: user.id, companyId: coStay.id }])
  await db.insert(userPermissions).values([
    { userId: user.id, companyId: coGone.id, permissionId: perm.id },   // scoped ที่จะถูกลบ — ต้องหาย
    { userId: user.id, companyId: coStay.id, permissionId: perm.id },   // บริษัทอื่น — ต้องอยู่
  ])
  expect((await req('DELETE', `/users/${user.id}/companies/${coGone.id}`, { authorization: superadmin })).status).toBe(200)
  expect((await db.select().from(userCompanies).where(eq(userCompanies.userId, user.id))).map(r => r.companyId)).toEqual([coStay.id])
  const up = await db.select().from(userPermissions).where(eq(userPermissions.userId, user.id))
  expect(up.length).toBe(1)
  expect(up[0].companyId).toBe(coStay.id)
  // idempotent: ลบซ้ำได้
  expect((await req('DELETE', `/users/${user.id}/companies/${coGone.id}`, { authorization: superadmin })).status).toBe(200)
})
