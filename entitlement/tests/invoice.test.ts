...
test('createInvoice สองครั้งติดกันของ tenant เดียวกัน → ทั้งคู่สำเร็จ เลขไม่ชนกัน (placeholder ไม่ชนกันเอง)', async () => {
  const { createInvoice } = await import('../src/modules/invoice/service')
  const tenant = await makeTenant()
  const a = await createInvoice(tenant.id, { description: 'ค่าบริการ A', amount: 100 })
  const b = await createInvoice(tenant.id, { description: 'ค่าบริการ B', amount: 200 })
  expect(a.number).toStartWith('INV-')
  expect(b.number).toStartWith('INV-')
