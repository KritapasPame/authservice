...
> **Product decision 2026-07-17: ขายเป็น subscription — ลูกค้า self-signup ไม่ใช่ invite-only**
> - สมัครแบบ personal → ระบบสร้างให้อัตโนมัติ: 1 tenant + 1 company + user เป็น admin ของ company ตัวเอง
> - สมัครแบบองค์กร/เครือ → tenant เดียว หลาย company (โครงสร้างเดิม)
> - **superadmin ไม่ได้สร้าง tenant ให้ลูกค้า** — มีไว้ ops/support เท่านั้น
> - ต้องมี **signup flow**: endpoint ที่ orchestrate สร้าง Zitadel org + user + tenant/company rows
>   (ใช้ primitives เดิมของ entitlement — `createTenant`/`createCompany`/`inviteUser` ครอบด้วย flow ใหม่)
>   → งานออกแบบแยก ยังไม่อยู่ในแผน Login V2 นี้
