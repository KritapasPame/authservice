...
## Implementation notes (หลัง final review 2026-07-19)

Deviations จาก spec (ตั้งใจ จดไว้):
- `GET /admin-ui/tenants/:id/users` → implement เป็น `GET /users/tenant/:tenantId` (prefix /admin เป็น superadmin-only ทั้ง router)
- `positions` ใน claims ไม่ทำ (YAGNI) — หน้า UI อ่านจาก `GET /users/tenant/:tenantId`
- signup ไม่รับ password — ใช้ Zitadel first-login flow เดิม (pretest ไม่มี SMTP)
- `POST /users/:id/companies` ไม่เช็ค company_limit — membership ไม่ได้กินโควตาบริษัท (spec เขียนเกิน)
- resolver: group admin กรอง company active เท่านั้น; membership ปกติไม่กรอง status (product decision ค้าง เหมือน overview/tenantDetail count)
- edge: group admin ใน tenant ที่ไม่มี company active → grants ว่าง → guard ไม่ผ่าน ต้องให้ superadmin แก้

Release gate ก่อนเปิด `/signup/personal` สู่ public prod (ตอนนี้ pre-test ใช้ได้):
- email verification flow (ตอนนี้สร้าง user แบบ email verified — ยืม path ของ invite)
- กัน TOCTOU email ซ้ำ + cleanup orphan เมื่อ fail กลางทาง + rate limiting
