...
**Console UI ตั้งได้หยาบสุดเป็นชั่วโมง** — 10 นาทีต้องยิง Admin API (PAT ops ตัวเดิม):

```bash
curl -s -X PUT https://authservice.edmcompany.co.th/admin/v1/settings/oidc \
  -H "Authorization: Bearer $ZITADEL_PAT" -H "Content-Type: application/json" \
  -d '{"accessTokenLifetime":"600s","idTokenLifetime":"600s","refreshTokenIdleExpiration":"2592000s","refreshTokenExpiration":"7776000s"}'
```

- **Access Token: 10 นาที (600s)** — เป็นกลไกหลักคุม staleness (revoke role / disable user
  มีผลตอน token หมดอายุ ไม่ใช่ทันที); id token ห้ามยาวกว่า access token
- Refresh Token: idle 30 วัน (2592000s) / absolute 90 วัน (7776000s) เป็นจุดเริ่ม
- verify: login ใหม่แล้ว decode token — `exp - iat` ต้อง = 600
...
- [x] service user + PAT ใช้งานได้
- [x] target + execution สร้างแล้ว, signing key อยู่ใน env entitlement
- [x] OIDC app = JWT access token (`ZITADEL_AUDIENCE` ต้อง = **Project ID** ใน `aud` ของ token)
- [x] access token TTL 10 นาที + refresh policy ตั้งแล้ว (2026-07-17 ผ่าน Admin API — verify `exp-iat=600`)
- [x] `oidc-pkce-test.py` เห็น claims ครบ (2026-07-17: superadmin ✓, provisioned tenant user ✓, ก่อน provision = `(none)` ✓)
- [x] อัปเดต `docs/PRETEST-AUTH-DEPLOYMENT.md` ส่วน "ยังไม่เสร็จ" หลังผ่าน
